...
// The dominant difficulty of a test's questions — drives the catalog filter.
function modalDifficulty(qs: MCQ[]): Difficulty {
  const counts: Record<Difficulty, number> = { EASY: 0, MEDIUM: 0, HARD: 0 };
  for (const q of qs) counts[q.difficulty]++;
  return (["EASY", "MEDIUM", "HARD"] as Difficulty[]).sort(
...
const testsConfig: Record<string, TestSeed[]> = Object.fromEntries(
  subjects.map((s) => {
    ...
    const tests: TestSeed[] = chunks.map((qs, i) => ({
      ...
      durationMin: Math.max(15, Math.round((qs.length * 36) / 60)),
    }));
    return [s.slug, tests];
  }),
);
...
async function main() {
  ...
  console.log("   Admin: admin@fiajobprep.com / Admin@FIA2024!");
  console.log("   Demo : demo@fiajobprep.com  / Demo@1234 (forces password change)");
  console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
}
